...
namespace AnimalShop.Controllers
{
    ...
    {
        private AdventureWorks3017Entities1 db = new AdventureWorks3017Entities1();

        // GET: People
        public ActionResult Index()
        {
            var people = db.People.Include(p => p.Employee).Include(p => p.BusinessEntity).Include(p => p.Password);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "NameStyle,Title,FirstName,MiddleName,LastName,Suffix,EmailPromotion,AdditionalContactInfo,Demographics,ModifiedDate")] Person person)
        {
            if (ModelState.IsValid)
            {
